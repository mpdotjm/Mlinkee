...
app.get('/api/providers', (req, res) => {
  const users = readJSON('users.json');
  // Filter for providers and map out sensitive data (like passwords, credentials)
  const providers = users
    .filter(u => u.role === 'provider')
    .map(u => ({
      id: u.id,
      name: u.name,
      email: u.email, // Kept for contact purposes visually later if needed
...
// GET /api/jobs/user — get current user's jobs
...
// POST /api/services — post a service (protected)
...
// GET /api/services/user — get current user's services
...
// POST /api/messages — send a message format
...
// POST /api/ai/match — Smart Job Matching
...
// POST /api/ai/generate — AI Generate/Improve Descriptions
...
// ============================================================
app.get('*', (req, res) => {
  // Serve the requested HTML file if it exists, otherwise index.html
...
// ============================================================
app.listen(PORT, () => {
  console.log(`\n  ✨ Linkee Backend running on http://localhost:${PORT}`);
